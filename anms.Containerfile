...
CMD ["bash", "-c", "if [ -n \"$DB_HOST\" ]; then for i in $(seq 1 60); do (echo > /dev/tcp/$DB_HOST/5432) 2>/dev/null && break; sleep 1; done; fi; exec refdm-proxy -l ${DTNMA_LOGLEVEL} -a ${AMP_PROXY_SOCKET}"]
EXPOSE 8089/tcp
