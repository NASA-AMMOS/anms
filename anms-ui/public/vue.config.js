...
(function () {
  ...
  const htmlMinifierOpts = {
    ignoreCustomComments: [/Johns Hopkins University Applied Physics Laboratory. All Rights Reserved/],
    removeComments: true,
  ...
  module.exports = {
    ...
    devServer: {
      quiet: false,
      stats: 'minimal',
      contentBase: [publicAppDir, publicAssetsDir],
      host: config.net.bindInterface,
      port: config.net.port + 1,
      before: function (app, server) {
        app.all('*', function (req, res) {
          res.redirect(302, config.ssl.enabled ? config.uris.sslWeb : config.uris.web); // 302 to prevent permanent...
        });
      },
      writeToDisk: true,
      hot: true, // set to false to disable hot-replacement
      // hotOnly: true, // uncomment to prevent browser refresh (also set hot to false)
      clientLogLevel: 'info',
      disableHostCheck: false,
      watchContentBase: false, // setting this to true will always trigger a full page reload...
    ...
    chainWebpack: config => {
      ...
      const jsOutputHashName = path.basename(config.output.get('chunkFilename'));
      config.output.set('filename', path.join(scriptsOutput, jsOutputName));
      config.output.set('chunkFilename', path.join(scriptsOutput, jsOutputHashName));
      if (isProduction) {
      ...
      // Adjust Images output path
      config.module
        .rule('images')
        .use('url-loader')
        .tap(options => {
          const outputName = path.basename(options.fallback.options.name);
          options.fallback.options.name = path.join(imagesOutput, outputName);
          return options;
        });
      // Adjust SVG output path
      config.module
        .rule('svg')
        .use('file-loader')
        .tap(options => {
          const outputName = path.basename(options.name);
          options.name = path.join(imagesOutput, outputName);
          return options;
        });
      // Add Worker Loader
      config.module
