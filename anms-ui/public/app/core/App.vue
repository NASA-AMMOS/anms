<template>
  <div v-cloak
    ...
    <b-navbar toggleable="lg"
      ...
      <b-collapse id="nav-collapse"
        ...
        <b-navbar-nav class="ml-auto">
          <b-nav-item id="tour-app-usernavbutton"
            :to="{ name: 'user' }">{{ userNameRemote }} <font-awesome-icon icon="fa-solid fa-user" /></b-nav-item>
          <b-nav-item id="tour-app-logoutnavbutton"
            :href="logoutResource">Logout <font-awesome-icon icon="fa-solid fa-arrow-right-from-bracket" />
          </b-nav-item>
        </b-navbar-nav>
      </b-collapse>
    </b-navbar>
    <b-container fluid
      class="d-flex flex-column flex-fill-100">
      <div>
        <b-row>
          <b-col cols="4"
            align="center"
            ...
            <b-button-group size="sm"
              ...
              <b-button variant="outline-success"
                @click="$router.push('/monitor')">Monitor</b-button>
              <b-button variant="outline-success"
                @click="$router.push('/agents')">Agents</b-button>
              <!-- <b-button variant="outline-success" @click="$router.push('/messaging')">Messaging</b-button> -->
              <b-button variant="outline-success"
                @click="$router.push('/build')">Build</b-button>
              <b-button variant="outline-success"
                @click="$router.push('/status')">
                <template v-if="serviceError">
                  <span class="text-danger">Status</span>
                  <b-badge v-if="hasUpdateError"
                    show
                    variant="danger">error</b-badge>
                  <b-badge v-else
                    show
                    variant="danger">{{ numberErrorServices }}</b-badge>
                  <b-spinner v-if="serviceLoading"
                    variant="info"
                    label="Loading..."
                    type="grow"
                    small></b-spinner>
                ...
                <template v-else>
                  ...
                  <b-badge v-else
                    show
                    variant="success">Good</b-badge>
                </template>
              </b-button>
              <b-button variant="outline-success"
                @click="$router.push('/adms')">Adms</b-button>
            </b-button-group>
          </b-col>
        </b-row>
      </div>
    ...
    <div v-for="(alert, index) in alerts"
      :key="index">
      <b-alert @dismissed="removeAlertAndUpdate(index)"
        :show="alert.visible"
        :variant="alert.type"
        dismissible>ALERT#{{
          alert.id }}: {{ alert.msg }}</b-alert>
    </div>
  </div>
</template>
