...
export default {
  ...
  methods: {
    ...
    apiUpdateUserProfile(userName, payload) {
      return axios.put(
        Constants.BASE_API_URL + "users/" + encodeURIComponent(userName),
        { data: payload },
        {
          headers: this.createAuthenticationHeader(),
        }
    ...
    apiEnableUser(userId) {
      return axios.patch(
        Constants.BASE_API_URL + "users/enable/" + encodeURIComponent(userId),
        {},
        {
          headers: this.createAuthenticationHeader(),
        }
    ...
    apiDisableUser(userId) {
      return axios.patch(
        Constants.BASE_API_URL + "users/disable/" + encodeURIComponent(userId),
        {},
        {
          headers: this.createAuthenticationHeader(),
        }
    ...
    apiToggleUserMfa(userId, newValue) {
      return axios.patch(
        Constants.BASE_API_URL + "users/mfa/" + encodeURIComponent(userId),
        { newState: newValue },
        {
          headers: this.createAuthenticationHeader(),
        }
    ...
    apiQueryForAgents(payload) {
      ...
      if (payload.searchString === "") {
        ...
        return axios.get(
          Constants.BASE_API_URL + `agents/search/${searchString}`,
          { params: params }
        );
      }
    },
    ...
    apiQueryForARIById(meta_id, obj_id) {
      return axios.get(
        Constants.BASE_API_URL + `build/ari/id/${meta_id}/${obj_id}`
      );
    },
    ...
    apiEntriesForReport(obj_agent_id, adm, report_name) {
      return axios.get(
        Constants.BASE_API_URL +
          `report/entries/table/${obj_agent_id}/${adm}/${report_name}`
      );
    },
    ...
    apiEntriesForOperations(agentId) {
      // get the names of crude operations
      return axios.get(
        Constants.BASE_API_URL + `agents/parameter/name/${agentId}`
      );
    },

    apiPutCRUD(agentId, optId, params) {
      return axios.put(
        Constants.BASE_API_URL + `agents/parameter/send/${agentId}/${optId}`,
        params
      );
    },
    ...
    apiBuildControl(nodeEID) {
      return (
        axios.put(Constants.BASE_API_URL + "nm/agents", nodeEID),
        { headers: {} }
      );
    },
    ...
    apiSendRawCommand(nodeEID, command) {
      return axios.put(
        Constants.BASE_API_URL + "nm/agents/eid/" + nodeEID + "/hex",
        { data: command }
      );
    },
    ...
    apiPrintAgentReports(nodeEID) {
      return axios.get(
        Constants.BASE_API_URL + "nm/agents/eid/" + nodeEID + "/reports/json"
      );
    },

    apiClearAgentReports(nodeEID) {
      return axios.put(
        Constants.BASE_API_URL + "nm/agents/eid/" + nodeEID + "/clear_reports"
      );
    },

    apiClearAgentTables(nodeEID) {
      return axios.put(
        Constants.BASE_API_URL + "nm/agents/eid/" + nodeEID + "/clear_tables"
      );
    },
    ...
    apiGetAlerts() {
      return axios.get(Constants.BASE_API_URL + "alerts/incoming", {
    ...
    apiGetServiceStatus() {
      return axios.get(Constants.BASE_API_URL + "core/service_status", {
        headers: { accept: "application/json" },
      });
    },
    ...
    apiPutTranscodedHex(cbor) {
      return axios.put(
        Constants.BASE_API_URL + "transcoder/ui/incoming/" + cbor + "/hex"
      );
    },

    apiPutTranscodedString(ari) {
      return axios.put(Constants.BASE_API_URL + "transcoder/ui/incoming/str", {
        ari: ari,
      });
    },
    ...
    apiQueryForTranscoderLog(payload) {
      ...
      if (payload.searchString === "") {
        return axios.get(Constants.BASE_API_URL + "transcoder/ui/log", {
          params: params,
        });
      } else {
        const searchString = encodeURIComponent(payload.searchString);
        return axios.get(
          Constants.BASE_API_URL + `transcoder/ui/log/search/${searchString}`,
          { params: params }
        );
      }
    },
    ...
    apiSendTBR(nodeEID, start, period, count, report, tbrCount) {
      ...
      startArr.forEach(
        (par) =>
          (startHexString = startHexString + zeroPad(par.toString(16), 2))
      );
      ...
      periodArr.forEach(
        (par) =>
          (periodHexString = periodHexString + zeroPad(par.toString(16), 2))
      );
      ...
      countArr.forEach(
        (par) =>
          (countHexString = countHexString + zeroPad(par.toString(16), 2))
      );
