  ...
  <b-row class="mt-3">
    <b-col cols="12">
      ...
      <div class="users">
        ...
        <b-modal id="modal-add-or-update-user" ref="addOrUpdateUserModal" :title="userModal.title"
                 @hidden="resetUserModal" @ok.prevent="handleUserSubmit">
          <form ref="addOrUpdateUserForm" @submit.stop.prevent="handleUserSubmit">
            <b-form-group :state="isUpdateModal ? null : userModal.formState.username" label="Username"
                          label-for="user-name-input"
                          invalid-feedback="Username is required">
              <b-form-input id="user-name-input" v-model="userModal.form.username"
                            :state="isUpdateModal ? null : userModal.formState.username"
                            :disabled="isUpdateModal"
                            required autocomplete="username"/>
            </b-form-group>
            <b-form-group :state="userModal.formState.firstName" label="First Name" label-for="first-name-input"
                          invalid-feedback="First name is required">
              <b-form-input id="first-name-input" v-model="userModal.form.firstName"
                            :state="userModal.formState.firstName" required/>
            </b-form-group>
            <b-form-group :state="userModal.formState.lastName" label="Last Name" label-for="last-name-input"
                          invalid-feedback="Last name is required">
              <b-form-input id="last-name-input" v-model="userModal.form.lastName" :state="userModal.formState.lastName"
                            required/>
            </b-form-group>
            <b-form-group :state="userModal.formState.email" label="Email" label-for="email-input"
                          invalid-feedback="Email address is required">
              <b-form-input id="email-input" v-model="userModal.form.email" :state="userModal.formState.email"
                            required/>
            </b-form-group>
            <b-form-group :state="userModal.formState.organization" label="Organization"
                          label-for="user-organization-input"
                          invalid-feedback="Organization is required">
              <b-form-input id="user-organization-input" v-model="userModal.form.organization"
                            :state="userModal.formState.organization" required/>
            </b-form-group>
            <b-form-group :state="userModal.formState.roles" label="Roles"
                          label-for="user-roles-input"
                          invalid-feedback="At least one role is required">
              <b-form-select id="user-roles-input" v-model="userModal.form.roles" :options="userRoles"
                             multiple :select-size="4" required/>
            </b-form-group>
            <b-form-group :state="userModal.formState.permissions" label="Permissions"
                          label-for="user-permissions-input">
              <b-form-select id="user-permissions-input" v-model="userModal.form.permissions"
                             :options="userPermissions" multiple :select-size="4" />
            </b-form-group>
            <b-form-group>
              <b-button size="sm" @click="userModal.form.permissions=[]">Clear Permissions</b-button>
            </b-form-group>
            <div v-if="userModal.title==='Add New User'">
              <b-form-group label="Password" label-for="user-password-input" :state="userModal.formState.password"
                                invalid-feedback="Password is required, must match repeat, and be longer than 7 characters">
                <b-input-group>
                  <b-form-input id="user-password-input" v-model="userModal.form.password"
                                :type="userModal.passwordRenderType" :state="userModal.formState.password"
                                required autocomplete="new-password" @input="verifyPasswords"/>
                  <b-input-group-append>
                    <b-button variant="light" @click="togglePasswordView">
                      <span v-show="userModal.passwordRenderType === 'password'" class="fa fa-eye"></span>
                      <span v-show="userModal.passwordRenderType !== 'password'" class="fa fa-eye-slash"></span>
                    </b-button>
                  </b-input-group-append>
                </b-input-group>
              </b-form-group>
              <b-form-group :state="userModal.formState.passwordRepeat" label="Repeat Password"
                            label-for="user-password-repeat-input"
                        invalid-feedback="Repeated Password is required and must match above password.">
                <b-form-input id="user-password-repeat-input" v-model="userModal.form.passwordRepeat"
                              :type="userModal.passwordRenderType" :state="userModal.formState.passwordRepeat"
                              :disabled="userModal.form.password === null" required autocomplete="new-password"
        ...
        <div id="tour-user-admin-add-users" class="float-left">
          <b-button variant="primary" size="sm" @click="openUserModal(modalActions.ADD, null)">
        ...
        <div id="tour-useradmin-toggledisabledusers" class="float-right">
          <b-button v-if="usersSelected.length > 0" v-b-modal.modal-assign-user style="margin-right: 0.35em" size="sm">Assign Users</b-button>
          <b-button v-else style="margin-right: 0.35em" disabled size="sm">Assign Users</b-button>
          <b-button v-if="!showDisabledUsers" size="sm" :disabled="!hasDisabledUsers" @click="toggleDisplayedUsers">
            Show Disabled
          </b-button>
          <b-button v-else size="sm" @click="toggleDisplayedUsers">
            Hide Disabled
          </b-button>
        </div>

        <div class="clearfix" style="margin-bottom: 0.35em"></div>

        <b-table ref="userTable" id="tour-useradmin-userstable" class="user-mgmt-table" responsive striped hover :fields="userFields" :items="displayedUsers"
                 selectable select-mode="range"
                 selected-variant="outline-light"
                 @row-selected="userRowSelected">
          <template v-slot:cell(actions)="data">
            <b-button size="sm" style="margin-right: 0.35em" @click="openUserModal(modalActions.EDIT, data.item.id)">
              Edit User
            </b-button>
            <b-button v-if="data.item.isEnabled" variant="danger" size="sm" style="margin-right: 0.35em"
                      :disabled="isFinalAdmin(data.item.roles)"
                      @click="disableUser(data.item.id)">
              Disable
            </b-button>
            <b-button v-else variant="success" size="sm" style="margin-right: 0.35em" @click="enableUser(data.item.id)">
              Enable
            </b-button>
            <b-button type="button" :variant="data.item.isMfaEnabled ? 'danger' : 'warning'" size="sm"
