...
AS $$ BEGIN
   ...
   SELECT obj_metadata_id INTO r_obj_id
    ...
    IF (r_obj_id IS NULL) THEN
    	INSERT INTO obj_metadata(data_type_id, obj_name, namespace_id) VALUES(p_obj_type_id, p_obj_name, p_namespace_id)RETURNING r_obj_id; 
...
BEGIN
	...
	INSERT INTO obj_formal_definition(obj_metadata_id, use_desc, obj_enum) VALUES(p_obj_metadata_id, p_use_desc, @obj_enum) Returning obj_formal_definition_id as r_formal_id ;
...
LANGUAGE plpgsql
AS $$ BEGIN
	INSERT INTO obj_actual_definition(obj_metadata_id, use_desc) VALUES(p_obj_metadata_id, p_use_desc) RETURNING obj_actual_definition_id as r_actual_id; 
...
LANGUAGE plpgsql
AS $$ BEGIN
	INSERT INTO ari_collection(num_entries, use_desc) VALUES(p_num_entries, p_use_desc) RETURNING ari_collection_id as r_ac_id; 
...
AS $$ BEGIN 
    ...
    LANGUAGE plpgsql*/
		INSERT INTO ari_collection_entry(ac_id, order_num) VALUES(p_ac_id, p_order_num) RETURNING ari_collection_entry_id as r_ac_entry_id;
...
LANGUAGE plpgsql
AS $$ BEGIN
	INSERT INTO actual_parmspec(fp_spec_id, tnvc_id, use_desc) VALUES(p_fp_spec_id, p_tnvc_id, p_use_desc) RETURNING actual_parmspec_id as r_ap_spec_id; 
...
AS $$ BEGIN 
    ...
    INSERT INTO registered_agents (agent_id_string,first_registered, last_registered)
		VALUES (@lower_name, @cur_time, @cur_time) RETURNING ower_name_id as r_registered_agents_id;
...
AS $$ BEGIN
    ...
    @data_id  = (SELECT data_type_id FROM data_type WHERE type_name  := p_data_type);
...
-- 	INSERT INTO data_value(value_converted, value_string) VALUES(p_value_converted, p_value_string) RETURNING INTO_id as r_value_id; 
...
AS $$ BEGIN
	IF( p_obj_id is Null AND p_obj_name is not NULL) THEN
		SET p_obj_id =  (select obj_id FROM obj_metadata WHERE obj_name = p_obj_name);
    ELSE 
		LEAVE edd_def_del;
    END IF;
...
AS $$ BEGIN 
	...
	-- IF p_out_type = 2 or p_out_type = 12 or p_out_type = 3 or p_out_type = 0 THEN BEGIN
			INSERT INTO expression(data_type_id, ac_id) VALUES(p_out_type, p_postfix_operations) RETURNING expression_id as r_expr_id; 
...
LANGUAGE plpgsql
AS $$ BEGIN
	INSERT INTO formal_parmspec(num_parms, use_desc) VALUES(p_num_parms, p_use_desc) RETURNING formal_parmspec_id as r_fp_spec_id; 
...
p_obj_definition_id) RETURNING RETURNING_id as r_fp_id;
...
p_modified_ts DATETIME, p_state integer, p_agent_id integer, INOUT r_set_id integer )
LANGUAGE plpgsql
AS $$ BEGIN
INSERT INTO outgoing_message_set (created_ts, modified_ts, state, agent_id)
VALUES (p_created_ts, p_modified_ts, p_state, p_agent_id) RETURNING VALUES (p_created_ts, p_modified_ts, p_state, p_agent_id) RETURNING as r_set_id;_id as r_set_id;
...
-- ==================================================================
CREATE OR REPLACE PROCEDURE SP__insert_outgoing_message_entry(IN p_set_id integer, p_message_order integer, p_start_ts datetime, p_ac_id integer, INOUT r_message_id integer)
LANGUAGE plpgsql
AS $$ BEGIN
INSERT INTO outgoing_message_entry (set_id, message_order, start_ts, ac_id)
VALUES (p_set_id, p_message_order, p_start_ts, p_ac_id) RETURNING VALUES (p_set_id, p_message_order, p_start_ts, p_ac_id) RETURNING as r_message_id;_id as r_message_id;
...
--      r_message_id integer - id of this message
-- ==================================================================
CREATE OR REPLACE PROCEDURE SP__insert_incoming_message_entry(IN p_set_id integer, p_message_order integer, p_start_ts datetime, p_ac_id integer, INOUT r_message_id integer)
LANGUAGE plpgsql
AS $$ BEGIN
...
AS $$ BEGIN
    ...
    INSERT INTO report_definition (ari_id, ts, tnvc_id) VALUES (p_ari_id, p_ts, p_tnvc_id) RETURNING report_definition_id as r_obj_id;
...
LANGUAGE plpgsql
AS $$ BEGIN
        INSERT INTO message_group_agents (group_id, agent_id) VALUES (p_group_id, p_agent) RETURNING message_group_agents_id as r_obj_id;
...
AS $$ BEGIN
        ...
        INSERT INTO message_group_agents (group_id, agent_id) VALUES (p_group_id, eid) RETURNING message_group_agents_id as r_obj_id;
...
AS $$ BEGIN
    ...
    IF (r_namespace_id IS NULL) THEN
    	INSERT INTO namespace(namespace_type, issuing_org, name_string, version_name) VALUES(p_namespace_type, p_issuing_org, p_name_string, p_version) RETURNING namespace_id as r_namespace_id; 
...
LANGUAGE plpgsql
AS $$ BEGIN
	INSERT INTO type_name_value_collection(num_entries, use_desc) VALUES(p_metadata_count, p_metadata_desc) RETURNING type_name_value_collection_id as r_tnvc_id; 
    
	@s := 'INSERT INTO type_name_value(tnvc_id, data_type, data_name, order_num) VALUES'; 
    @loops := 1; 
    WHILE @loops < p_metadata_count DO BEGIN
			-- @metadata_type
...
LANGUAGE plpgsql
AS $$ BEGIN
	IF (p_str IS NOT NULL) && (p_start_position IS NOT NULL)THEN BEGIN
			...
			WHILE (@position_index != @length) && (@delimiter_found != TRUE) DO BEGIN
...
p_wait_per time, p_run_count biginteger, p_start_time time, p_ac_id integer, INOUT r_definition_id integer)
LANGUAGE plpgsql
AS $$ BEGIN
	CALL SP__insert_obj_actual_definition(p_obj_id, p_use_desc, r_definition_id); 
...
LANGUAGE plpgsql
AS $$ BEGIN
	INSERT INTO type_name_value_collection(use_desc) VALUES(p_use_desc) RETURNING type_name_value_collection_id as r_tnvc_id;
...
p_data_name) RETURNING RETURNING_id as r_tnvc_entry_id;
...
CREATE OR REPLACE PROCEDURE SP__insert_tnvc_uvast_entry(IN p_tnvc_id integer, p_order_num integer,  p_data_name varchar,  p_entry_value biginteger , INOUT r_tnvc_entry_id integer )
LANGUAGE plpgsql
AS $$ BEGIN
...
CREATE OR REPLACE PROCEDURE SP__insert_tnvc_tv_entry(IN p_tnvc_id integer, p_order_num integer,  p_data_name varchar,  p_entry_value biginteger , INOUT r_tnvc_entry_id integer )
LANGUAGE plpgsql
AS $$ BEGIN
...
CREATE OR REPLACE PROCEDURE SP__insert_tnvc_ts_entry(IN p_tnvc_id integer, p_order_num integer,  p_data_name varchar,  p_entry_value biginteger , INOUT r_tnvc_entry_id integer )
LANGUAGE plpgsql
AS $$ BEGIN
...
CREATE OR REPLACE PROCEDURE SP__insert_tnvc_real64_entry(IN p_tnvc_id integer, p_order_num integer,  p_data_name varchar,  p_entry_value double, INOUT r_tnvc_entry_id integer)
LANGUAGE plpgsql
AS $$ BEGIN
...
CREATE OR REPLACE PROCEDURE SP__insert_tnvc_byte_entry(IN p_tnvc_id integer, p_order_num integer,  p_data_name varchar,  p_entry_value tinyint, INOUT r_tnvc_entry_id integer )
LANGUAGE plpgsql
AS $$ BEGIN
